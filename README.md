...
## Description
This repo has a set of scripts which were developed by me (and ChatGPT :lol:) in the process of scanning my family's old photo archive located on film. The main goal of these scripts was to automate things like EXIF metadata insertion and cropping out excess margins of the scanner.

Since this was an archival task all these scripts are tailored to use 48bit TIFF images. 24bit TIFF should work just fine. Other image formats may work but not tested by me at all. And since typically there are lots of photos in an archive like this all scripts works on per-directory basis <ins>not</ins> per-file one.
...
- **_xmp-extractor.py_** - extracts XMP tag contents (which contains ACR develop settings) into sidecar xmp file. Usefull to fix shitty Adobe policy of storing develop settings exclusively into original TIFF files wrecking EXIF data in the process.
...
2. Update metadata hierarchicaly:
...
6. For tags which value was set to `Marker.AUTO` actual value will be defined by the script logic - `ShutterSpeedValue` from `ExposureTime`, `ApertureValue` from `FNumber`, `ModifyDate` from corrent datetime and so on.
...
##### in metafile:
Tag name and value are set by `<tag_name> = <tag_value>` pair on a dedicated line. All lines starting with '#' are ignored. Markers can also be used here by assigning their enum values. For example `MakerNotes:All = <DELETE>` will set `MakerNotes:All` tag to `Marker.DELETE` value which will delete it from the resulting file.
...
<ins>For example:</ins> `1337-01_S1-1_C82-126-4096-2656_F25_O8_D1985-10-26-01-21@-7_G34.006100,-117.930600.tif`
...
<ins>For example:</ins> `C:\scan\output\{Make} {Model}\{Extra:FilmID}\{Extra:FilmID}-{Extra:FilmFrameNumber?02d}.{Extra:FileNameExtension}`
